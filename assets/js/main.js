...
(function () {
  ...
  (function setProfile() {
    ...
    if (socials) {
      socials.innerHTML = "";
      profileInfo.social.forEach((social) => {
        socials.innerHTML += `<a href='${social.url}' class='${social.class}' target='_blank'><i class='${social.icon}'></i></a>`;
      });
    }
    // About
    const aboutEls = select(".my-id-info", true);
    if (aboutEls) {
      const aboutInfo = profileInfo.getAbout();
      aboutEls.forEach((el) => {
        let dataIdInfo = el.getAttribute("data-id-info").split(",");
        let html = "";
        dataIdInfo.forEach((info) => {
          info = info.trim();
